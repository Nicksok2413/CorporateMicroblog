...
@pytest.fixture
def like_service(mock_like_repo: MagicMock, mock_tweet_repo: MagicMock) -> LikeService:
...
async def test_like_tweet_success(
    ...
    mock_like_repo.add_like.return_value = Like(
        user_id=test_user_obj.id, tweet_id=test_tweet_obj.id
    )

    # Вызываем метод сервиса
    await like_service.like_tweet(
        db=mock_db_session, current_user=test_user_obj, tweet_id=test_tweet_obj.id
    )

    # Проверяем вызовы
    mock_tweet_repo.get.assert_awaited_once_with(
        mock_db_session, obj_id=test_tweet_obj.id
    )
    mock_like_repo.add_like.assert_awaited_once_with(
        mock_db_session, user_id=test_user_obj.id, tweet_id=test_tweet_obj.id
    )
...
async def test_like_tweet_tweet_not_found(
    ...
    with pytest.raises(NotFoundError):
        await like_service.like_tweet(
            db=mock_db_session, current_user=test_user_obj, tweet_id=tweet_id
        )
...
async def test_unlike_tweet_success(
    ...
    # Вызываем метод сервиса
    await like_service.unlike_tweet(
        db=mock_db_session, current_user=test_user_obj, tweet_id=tweet_id
    )

    # Проверяем вызовы
    mock_like_repo.get_like.assert_awaited_once_with(
        mock_db_session, user_id=test_user_obj.id, tweet_id=tweet_id
    )
    mock_like_repo.delete_like.assert_awaited_once_with(
        mock_db_session, user_id=test_user_obj.id, tweet_id=tweet_id
    )
    mock_db_session.commit.assert_awaited_once()  # Должен быть коммит
    mock_db_session.rollback.assert_not_awaited()  # Роллбэка быть не должно
