...
# --- Тесты для POST /api_old/v1/media ---
...
async def test_upload_media_success_jpeg(client: AsyncClient, auth_headers_user1: dict):
    ...
    response = await client.post("/api_old/v1/media", files=files, headers=auth_headers_user1)
...
async def test_upload_media_no_file(client: AsyncClient, auth_headers_user1: dict):
    response = await client.post("/api_old/v1/media", headers=auth_headers_user1)  # No files payload
...
async def test_upload_media_unauthorized(client: AsyncClient):
    file_content = b"fake jpeg data"
    files = {"file": ("test.jpg", io.BytesIO(file_content), "image/jpeg")}
    response = await client.post("/api_old/v1/media", files=files)  # No headers
...
async def test_upload_media_invalid_key(client: AsyncClient):
    ...
    response = await client.post("/api_old/v1/media", files=files, headers=headers)
