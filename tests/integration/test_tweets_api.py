...
# --- Тесты для POST /api_old/v1/tweets ---
...
async def test_create_tweet_success_text_only(client: AsyncClient, test_user1: User, auth_headers_user1: dict):
    ...
    response = await client.post("/api_old/v1/tweets", json=tweet_data, headers=auth_headers_user1)
...
async def test_create_tweet_unauthorized(client: AsyncClient):
    ...
    response = await client.post("/api_old/v1/tweets", json=tweet_data)  # No headers
...
async def test_create_tweet_invalid_key(client: AsyncClient):
    ...
    response = await client.post("/api_old/v1/tweets", json=tweet_data, headers=headers)
    assert response.status_code == status.HTTP_403_FORBIDDEN


# --- Тесты для GET /api_old/v1/tweets ---
...
async def test_get_feed_success_empty(client: AsyncClient, test_user3_no_tweets: User):
    ...
    response = await client.get("/api_old/v1/tweets", headers=headers)
...
async def test_get_feed_unauthorized(client: AsyncClient):
    response = await client.get("/api_old/v1/tweets")  # No headers
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
...
# --- Тесты для DELETE /api_old/v1/tweets/{tweet_id} ---
...
async def test_delete_tweet_success(client: AsyncClient, test_user1: User, test_tweet_user1: Tweet,
    ...
    response = await client.delete(f"/api_old/v1/tweets/{test_tweet_user1.id}", headers=auth_headers_user1)
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    assert data["result"] is True
...
async def test_delete_tweet_forbidden(client: AsyncClient, test_user2: User, test_tweet_user1: Tweet,
    ...
    response = await client.delete(f"/api_old/v1/tweets/{test_tweet_user1.id}", headers=auth_headers_user2)
    assert response.status_code == status.HTTP_403_FORBIDDEN
    data = response.json()
    assert data["result"] is False
...
async def test_delete_tweet_not_found(client: AsyncClient, test_user1: User, auth_headers_user1: dict):
    response = await client.delete("/api_old/v1/tweets/9999", headers=auth_headers_user1)
    assert response.status_code == status.HTTP_404_NOT_FOUND
    data = response.json()
    assert data["result"] is False
    assert data["error_type"] == "not_found"
...
async def test_delete_tweet_unauthorized(client: AsyncClient, test_tweet_user1: Tweet):
    response = await client.delete(f"/api_old/v1/tweets/{test_tweet_user1.id}")  # No headers
    assert response.status_code == status.HTTP_401_UNAUTHORIZED


async def test_delete_tweet_invalid_key(client: AsyncClient, test_tweet_user1: Tweet):
    headers = {"api-key": "invalidkey"}
    response = await client.delete(f"/api_old/v1/tweets/{test_tweet_user1.id}", headers=headers)
    assert response.status_code == status.HTTP_403_FORBIDDEN
