...
# --- Тесты для POST /api_old/v1/users/{user_id}/follow ---

async def test_follow_user_success(client: AsyncClient, test_user1: User, test_user2: User, auth_headers_user1: dict,
                                   db_session: AsyncSession):
    response = await client.post(f"/api_old/v1/users/{test_user2.id}/follow", headers=auth_headers_user1)
...
async def test_follow_user_self(client: AsyncClient, test_user1: User, auth_headers_user1: dict):
    response = await client.post(f"/api_old/v1/users/{test_user1.id}/follow", headers=auth_headers_user1)
...
async def test_follow_user_not_found(client: AsyncClient, test_user1: User, auth_headers_user1: dict):
    response = await client.post("/api_old/v1/users/9999/follow", headers=auth_headers_user1)
...
async def test_follow_user_unauthorized(client: AsyncClient, test_user2: User):
    response = await client.post(f"/api_old/v1/users/{test_user2.id}/follow")  # No headers
    assert response.status_code == status.HTTP_401_UNAUTHORIZED


async def test_follow_user_invalid_key(client: AsyncClient, test_user2: User):
    headers = {"api-key": "invalidkey"}
    response = await client.post(f"/api_old/v1/users/{test_user2.id}/follow", headers=headers)
    assert response.status_code == status.HTTP_403_FORBIDDEN


# --- Тесты для DELETE /api_old/v1/users/{user_id}/follow ---

async def test_unfollow_user_success(client: AsyncClient, test_user1: User, test_user2: User,
                                     test_follow_user1_on_user2: Follow, auth_headers_user1: dict,
                                     db_session: AsyncSession):
    response = await client.delete(f"/api_old/v1/users/{test_user2.id}/follow", headers=auth_headers_user1)
...
async def test_unfollow_user_self(client: AsyncClient, test_user1: User, auth_headers_user1: dict):
    response = await client.delete(f"/api_old/v1/users/{test_user1.id}/follow", headers=auth_headers_user1)
...
async def test_unfollow_user_not_found(client: AsyncClient, test_user1: User, auth_headers_user1: dict):
    response = await client.delete("/api_old/v1/users/9999/follow", headers=auth_headers_user1)
    assert response.status_code == status.HTTP_404_NOT_FOUND
    data = response.json()
    assert data["result"] is False
    assert data["error_type"] == "not_found"
...
async def test_unfollow_user_unauthorized(client: AsyncClient, test_user2: User):
    response = await client.delete(f"/api_old/v1/users/{test_user2.id}/follow")  # No headers
    assert response.status_code == status.HTTP_401_UNAUTHORIZED


async def test_unfollow_user_invalid_key(client: AsyncClient, test_user2: User):
    headers = {"api-key": "invalidkey"}
    response = await client.delete(f"/api_old/v1/users/{test_user2.id}/follow", headers=headers)
    assert response.status_code == status.HTTP_403_FORBIDDEN
