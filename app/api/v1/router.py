...
router_v1.include_router(follow.router)  # Префикс не указываем так как полный путь `/users/{id}/follow` уже задан в follow.router
router_v1.include_router(likes.router)  # Префикс не указываем так как полный путь `/tweets/{id}/likes` уже задан в likes.router
