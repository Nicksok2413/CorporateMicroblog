...
class Settings(BaseSettings):
    ...
    POSTGRES_HOST: str = Field("db", description="Хост PostgreSQL (имя сервиса в Docker)") # Изменено на 'db' для Docker
    POSTGRES_PORT: int = Field(5432, description="Порт PostgreSQL")
    # Используем строку как тип по умолчанию для AnyUrl, Pydantic разберет ее
    ...
    def model_post_init(self, __context) -> None:
        ...
        try:
            ...
            print(f"Warning: Не удалось создать директорию медиа {self.STORAGE_PATH_OBJ}: {e}")
        ...
        if self.LOG_FILE:
            try:
                ...
                print(f"Warning: Не удалось создать директорию для лог-файла {self.LOG_FILE.parent}: {e}")
...
print(f"Эффективный URL БД: {'*' * 5}{settings.EFFECTIVE_DATABASE_URL[-20:]}" if settings.EFFECTIVE_DATABASE_URL else "Not Set")
