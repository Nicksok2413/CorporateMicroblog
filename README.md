...
4.  **Проверка:**
    ...
    *   HTTP на `http://localhost:8000` будет редиректить на HTTPS.
    ...
    *   Prometheus UI будет доступен по адресу `http://localhost:9090`.
    *   Grafana UI будет доступен по адресу `http://localhost:3000` (логин/пароль по умолчанию: `admin`/`admin`)
