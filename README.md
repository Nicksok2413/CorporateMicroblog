...
*   **Мониторинг Ошибок:** Интеграция с Sentry (опционально, через `SENTRY_DSN`) для отслеживания ошибок и производительности.
...
*   **Прочее:** aiofiles, python-multipart, python-dotenv, su-exec
...
2.  **Создайте файл переменных окружения:**
    ...
    **(Опционально):** Чтобы включить отправку ошибок и данных о производительности в Sentry, раскомментируйте и установите переменную `SENTRY_DSN`, указав DSN вашего проекта Sentry. Если `SENTRY_DSN` не задан, интеграция Sentry будет отключена.
...
3.  **Запустите с помощью Docker Compose:**
    ...
    Копирование начальных медиафайлов будет выполнено автоматически при старте сервиса `media-initializer` (отработает и завершится).
    При первом запуске Nginx автоматически сгенерирует самоподписанный SSL-сертификат.
...
4.  **Проверка:**
    ...
    *   Если зайдете на `http://localhost:8000`, то вас должно автоматически перенаправить на `https://localhost:8443`.
