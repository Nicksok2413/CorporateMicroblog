...
*   **Nginx** (`nginx` сервис) выступает в роли входной точки (реверс-прокси), слушает порт 8000 (HTTP, с редиректом на HTTPS) и порт 8443 (HTTPS) на хост-машине. Он отвечает за:
    *   Раздачу статических файлов пользовательского интерфейса (из `src/static/`) по HTTPS.
    *   Раздачу загруженных медиафайлов (изображений) по пути `/media/` по HTTPS.
    *   Проксирование всех API-запросов (начинающихся с `/api/`) на бэкенд-сервис FastAPI по HTTPS.
*   **FastAPI** (`web` сервис) работает под управлением Uvicorn, обрабатывает исключительно API-запросы (`/api/...`), взаимодействуя с базой данных PostgreSQL через асинхронный SQLAlchemy и Alembic для миграций.
*   **PostgreSQL** (`db` сервис) хранит все данные приложения.
*   **Аутентификация** пользователей предполагается внешней, бэкенд работает с предоставленным `api-key` в заголовке запроса к API. Для повышения безопасности API-ключи пользователей хранятся в базе данных в хешированном виде (Argon2 + SHA256). Дополнительно, на уровне Nginx настроено ограничение частоты запросов (Rate Limiting) для защиты от подбора ключей.
...
4.  **Запустите с помощью Docker Compose:**
    Эта команда соберет образы (если необходимо) и запустит три контейнера: `db`, `web`, `nginx`. Nginx будет слушать порт **8000** (HTTP, редирект на HTTPS) и **8443** (HTTPS) на вашем хосте. Миграции базы данных и копирование начальных медиафайлов будут выполнены автоматически при старте сервиса `web`.
    ```bash
...
5.  **Проверка:**
    *   Веб-интерфейс будет доступен по адресу `https://localhost:8443` (возможно, потребуется принять самоподписанный сертификат в браузере).
...
Все API-эндпоинты (начинающиеся с `/api/...`) требуют передачи API-ключа пользователя в HTTP-заголовке `api-key`. Ключи хранятся в хешированном виде, используется Rate Limiting.
...
```bash
curl -k -X GET "https://localhost:8443/api/users/me" -H "accept: application/json" -H "api-key: test"
```
