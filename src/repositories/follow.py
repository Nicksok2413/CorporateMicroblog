...
class FollowRepository:
    ...
    async def get_follow(
        ...
        """
        log.debug(
            f"Проверка подписки: follower_id={follower_id}, following_id={following_id}"
        )
    ...
    async def add_follow(
        ...
        """
        log.debug(
            f"Подготовка к добавлению подписки: follower_id={follower_id}, following_id={following_id}"
        )
    ...
    async def delete_follow(
        ...
        """
        log.debug(
            f"Подготовка к удалению подписки: follower_id={follower_id}, following_id={following_id}"
        )
    ...
    async def get_following_ids(
        self, db: AsyncSession, *, follower_id: int
    ) -> List[int]:
        """
        ...
        statement = select(self.model.following_id).where(
            self.model.follower_id == follower_id
        )
        result = await db.execute(statement)
    ...
    async def get_following_with_users(
        self, db: AsyncSession, *, follower_id: int
    ) -> Sequence[Follow]:
        """
        ...
        """
        log.debug(
            f"Получение подписок с деталями пользователей для follower_id={follower_id}"
        )
        ...
        statement = (
            ...
            .options(
                selectinload(Follow.followed_user)
            )  # Загружаем профиль того, на кого подписан
        )

        result = await db.execute(statement)
        return result.scalars().all()

    async def get_followers_with_users(
        self, db: AsyncSession, *, following_id: int
    ) -> Sequence[Follow]:
        """
        ...
        """
        log.debug(
            f"Получение подписчиков с деталями пользователей для following_id={following_id}"
        )
