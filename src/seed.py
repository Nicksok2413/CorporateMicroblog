...
from src.repositories import (follow_repo, like_repo,
                              user_repo)

# Добавляем путь к проекту, чтобы можно было импортировать из src
...
async def seed_data():
    ...
    try:
        ...
        # нужно либо пробросить порт (как у нас 5433), либо запускать seed внутри контейнера src.
